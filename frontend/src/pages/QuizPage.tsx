...
export function QuizPage() {
  ...
  useEffect(() => {
    ...
    quizApi.getOrGenerateQuestions(slug, provider, key)
      ...
      .catch(async (err: unknown) => {
        ...
        if (msg === 'already_attempted') {
          ...
          try {
            ...
            if (cd.active) setCooldownUntil(new Date(cd.next_claim))
          } catch { setError('Cooldown active.') }
        } else if (!key) {
  ...
  async function submit() {
    ...
    try {
      const result = await quizApi.submitAttempt(
        slug,
        questions.map((q) => ({ question_id: q.id, chosen_answer: answers[q.id] ?? '' }))
      )
  ...
  if (loading) return (
    <div style={{ textAlign: 'center', marginTop: 80, color: 'var(--text-muted)' }}>
      Generating questions<span style={{ animation: 'none' }}>...</span>
    </div>
  )
  ...
  if (cooldownUntil) return (
    <div className="page-narrow" style={{ marginTop: 40 }}>
      <div className="cooldown-banner">
        ...
        <br />
        <span className="text-sm" style={{ marginTop: 6, display: 'block' }}>
          Next claim available: <strong>{cooldownUntil.toLocaleTimeString()}</strong>
        </span>
      </div>
  ...
  return (
    <div className="page">
      <div style={{ marginBottom: 24 }}>
        <h2 style={{ fontSize: '1.5rem', fontWeight: 800 }}>Quiz</h2>
        <p className="text-sm text-muted" style={{ marginTop: 4 }}>
          {answered}/{total} answered · 100% required to own this article
        </p>
        <div style={{
          marginTop: 8, height: 4, background: 'var(--border)', borderRadius: 999, overflow: 'hidden'
        }}>
          <div style={{
          ...
          <div className="quiz-options">
            {q.options.map((opt) => (
              ...
              >
                ...
                />
                {q.type === 'multiple_choice' ? `${opt.label}. ${opt.value}` : opt.value}
              </label>
            ))}
          </div>
        </div>
      ...
      <button
        ...
        style={{ marginTop: 8 }}
      >
        {submitting ? 'Submitting...' : answered < total ? `Answer ${total - answered} more` : 'Submit Answers'}
      </button>
    </div>
  )
}
